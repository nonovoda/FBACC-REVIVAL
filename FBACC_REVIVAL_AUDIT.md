...
## 5) Дизайн-система для реального редизайна (обязательное правило)

При реализации UI использовать файлы фирменного стиля из репозитория как source of truth:
- `UI_STYLE_GUIDE.md`
- `tokens.js`
- `typography.css`
- `styles.css`
...
Правила применения:
- не перепридумывать цвета/отступы/типографику вручную, а брать существующие токены и CSS-паттерны;
- сохранять compact dark utility стиль и русскую локализацию интерфейса;
- все долгие операции обязательно выводить в лог-блок в модалке.

---

## 6) Приоритетный roadmap (implementation-first)
